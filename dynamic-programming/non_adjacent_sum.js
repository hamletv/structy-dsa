...
// naive solution
const nonAdjacentSum = numbers => {
    // if current length of array given is 0 return 0.
    if(numbers.length === 0) return 0;

    // start from left to right, take first number and slice up until number not
    // adjacent to the first number - include/add.
    // do not include number that is next to the include value.
    const include = numbers[0] + nonAdjacentSum(numbers.slice(2));
    const exclude = nonAdjacentSum(numbers.slice(1));
    return Math.max(include, exclude);
};


const nonAdjacentSumMemo = (numbers, i=0, memo={}) => {
    if(i in memo) return memo[i];
    if(i >= numbers.length) return 0;
    const include = numbers[i] + nonAdjacentSumMemo(numbers, i+2);
    const exclude = nonAdjacentSumMemo(numbers, i+1);
