...
const lowestCommonAncestor = (root, val1, val2) => {
    const res1 = getPathHelper(root, val1);
    const res2 = getPathHelper(root, val2);
    const res1Set = new Set(res1);

    for(let val of res2){
        if(res1Set.has(val)) return val;
    }
};


const getPathHelper = (root, val) => {
    if(root === null) return null;
    if(root.val === val) return [root.val];

    const left = getPathHelper(root.left, val);
    if(left !== null) left.push(root.val);

    const right = getPathHelper(root.right, val);
    if(right !== null) right.push(root.val);
