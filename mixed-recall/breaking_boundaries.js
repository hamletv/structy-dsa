...
const breakingBoundaries = (m, n, k, r, c) => {
    const rowInbound = 0 <= r && r < m;
    const colInbound = 0 <= c && c < n;

    if(!rowInbound || !colInbound) return 1;
    if(k === 0) return 0;
    ...
    for(let delta of deltas){
        ...
        total += breakingBoundaries(m, n, k-1, r+dRow, c+dCol);
    }
    return total;
};


const breakingBoundariesMemo = (m, n, k, r, c, memo={}) => {
    const key = `${k},${r},${c}`;
    const rowInbound = 0 <= r && r < m;
    const colInbound = 0 <= c && c < n;
    ...
    for(let delta of deltas){
        const [dRow, dCol] = delta;
        total += breakingBoundariesMemo(m, n, k-1, r+dRow, c+dCol, memo);
    }
    memo[key] = total;
    return total;
};
